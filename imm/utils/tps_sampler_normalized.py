"""
TensorFlow-identical TPS sampler with normalized coordinate sampling.

This implementation ensures exact coordinate space matching with the original
TensorFlow implementation by sampling TPS parameters in normalized [-1,1] space.
"""
...
def sample_tps_w_normalized(vertical_points: int, horizontal_points: int, 
                           warpsd: Tuple[float, float], rotsd: float = 0.0, 
                           scalesd: float = 0.0, transsd: float = 0.1) -> np.ndarray:
    """
    Sample TPS transformation parameters in normalized [-1,1] coordinate space.
    
    This exactly matches the TensorFlow implementation's coordinate sampling.
    KEY DIFFERENCE: All sampling happens in normalized space, not pixel space!
    """
    n_points = vertical_points * horizontal_points
    
    # CRITICAL: Create control point grid in NORMALIZED [-1, 1] space (like TensorFlow)
    ...
    # Sample transformation parameters in NORMALIZED space
    warp_std = np.random.uniform(warpsd[0], warpsd[1])
    
    # Global transformation parameters (in normalized space)
    angle = np.random.normal(0, rotsd) * np.pi / 180.0
    scale = 1.0 + np.random.normal(0, scalesd)
    trans_y = np.random.normal(0, transsd)  # Already in [-1,1] relative units
    trans_x = np.random.normal(0, transsd)
    
    # Apply global transformation in normalized space
    ...
    # CRITICAL: Add local warping in normalized space (not pixel space!)
    # This ensures warp strength is independent of image resolution
    local_warp = np.random.normal(0, warp_std, (n_points, 2))
    transformed_points += local_warp
    
    # Build TPS system matrix in normalized coordinate space
    distances = ssd.cdist(control_points, control_points)
    
    # TPS kernel with same epsilon as TensorFlow
    real_min = 1e-8
    distances_clipped = np.clip(distances, real_min, None)
    tps_kernel = np.log(distances_clipped) * distances_clipped
    
    # Build system matrix components exactly as TensorFlow
    ones_col = np.ones((n_points, 1))
    
    # Top part: [K, 1, y, x] (note: y,x order matches TF)
    top_left = tps_kernel
    top_right = np.concatenate([ones_col, control_points], axis=1)
    top = np.concatenate([top_left, top_right], axis=1)
    
    # Bottom part: [1^T, y^T, x^T, 0]  
    bottom_left = np.concatenate([ones_col.T, control_points.T], axis=0)
    bottom_right = np.zeros((3, 3))
    bottom = np.concatenate([bottom_left, bottom_right], axis=1)
    
    # Complete system matrix
    system_matrix = np.concatenate([top, bottom], axis=0)
...
class TPSGridGenNormalized(nn.Module):
    """TPS grid generator working in normalized coordinate space."""
    
    def __init__(self, Ho: int, Wo: int, Hc: int, Wc: int):
        super(TPSGridGenNormalized, self).__init__()
        
        self._grid_hw = (Ho, Wo)
        self._cp_hw = (Hc, Wc)
        
        # CRITICAL: Initialize grid in NORMALIZED [-1, 1] space (not pixel space)
        ...
        # CRITICAL: Initialize control points in NORMALIZED [-1, 1] space
        ...
        # Create TPS kernel exactly as TensorFlow
        real_min = 1e-8
    ...
    def forward(self, W: torch.Tensor) -> torch.Tensor:
        ...
        # Reshape to grid format - already in [-1, 1] range for grid_sample
        Ho, Wo = self._grid_hw
...
class TPSRandomSamplerNormalized(nn.Module):
    """
    TensorFlow-identical TPS random sampler using normalized coordinate sampling.
    
    KEY IMPROVEMENT: All TPS sampling happens in normalized [-1,1] coordinate space,
    ensuring identical data distribution to TensorFlow regardless of image resolution.
    """
    ...
    def __init__(self, height: int, width: int, vertical_points: int = 10, 
                 ...
                 pad: bool = True):
        super(TPSRandomSamplerNormalized, self).__init__()
        ...
        # Use normalized coordinate TPS grid generator
        self.tps = TPSGridGenNormalized(
    ...
    def _sample_grid(self) -> torch.Tensor:
        """Sample transformation grid in normalized coordinate space."""
        W = sample_tps_w_normalized(
            self.vertical_points, self.horizontal_points, self.warpsd,
            self.rotsd, self.scalesd, self.transsd)
        W = torch.from_numpy(W.astype(np.float32))
        
        # Generate grid in normalized space
    ...
    def forward(self, input_tensor: torch.Tensor, training: bool = True) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Apply TPS transformation using normalized coordinate sampling.
        
        This matches TensorFlow's coordinate space exactly, ensuring identical
        data distribution regardless of image resolution.
        """
        if not training:
        ...
        # Get transformation grids (already in [-1, 1] space)
        ...
        # Grid is already in normalized [-1, 1] space as expected by grid_sample
    ...
    def forward_py(self, input_np: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            # Apply transformation in normalized coordinate space
