...
def train_loop(opts, graph, loss, train_dataset, training_pl, handle_pl, train_op,
  ...
  with graph.as_default(), tf.device('/cpu:0'):
    ...
    for step in xrange(start_step, num_steps):
      ...
      # Optional: print scalar loss each step
      if opts.get('print_loss_each_step', False):
        try:
          print('Loss (step {}): {:.6f}'.format(step, float(loss_value)))
        except Exception:
          pass
      # Optional: print ranges of tensors in the 'tensors' collection
      if opts.get('print_tensor_ranges', False):
        every = int(opts.get('tensor_ranges_every', 1)) if isinstance(opts.get('tensor_ranges_every', 1), int) else 1
        if every <= 0:
          every = 1
        if step % every == 0:
          try:
            tensors = tf.get_collection('tensors')
            if tensors:
              feed_dbg = {handle_pl: train_handle, training_pl: False}
              print('\n=== TENSOR RANGES AT STEP {} ==='.format(step))
              for item in tensors:
                try:
                  name, tensor = item
                  val = session.run(tensor, feed_dict=feed_dbg)
                  if isinstance(val, np.ndarray):
                    print('{}: shape={}, min={:.6f}, max={:.6f}, mean={:.6f}, std={:.6f}'.format(
                      name, val.shape, float(np.min(val)), float(np.max(val)), float(np.mean(val)), float(np.std(val))))
                  else:
            ...
            pass
      # Print all trainable weights if requested
      if opts.get('print_weights_every_step', False):
        print('\n=== WEIGHTS AT STEP {} ==='.format(step))
        for var in tf.trainable_variables():
          try:
            var_value = session.run(var)
            vshape = var.shape
            vmean = float(np.mean(var_value))
            vstd = float(np.std(var_value))
            vnorm = float(np.linalg.norm(var_value))
            print('{}: shape={}, mean={:.6f}, std={:.6f}, norm={:.6f}'.format(
              var.name, vshape, vmean, vstd, vnorm))
          except Exception as e:
            print('{}: Error printing variable - {}'.format(var.name, e))
